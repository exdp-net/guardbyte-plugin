...
public final class HAProxyMessage {
    ...
    static HAProxyMessage decodeHeader(ByteBuf header) {
        ...
        try {
            String versionString = String.valueOf((char) verCmdByte);
            ver = HAProxyProtocolVersion.valueOf(versionString);
        ...
        try {
            String commandString = String.valueOf((char) verCmdByte);
            cmd = HAProxyCommand.valueOf(commandString);
        } catch (IllegalArgumentException e) {
            throw new HAProxyProtocolException(e);
        }
        ...
        try {
            protAndFam = HAProxyProxiedProtocol.valueOf(String.valueOf(header.readByte()));
        } catch (IllegalArgumentException e) {
            throw new HAProxyProtocolException(e);
        }
        ...
        if (addressFamily == HAProxyProxiedProtocol.AddressFamily.AF_UNIX) {
            ...
            int addressEnd = header.forEachByte(startIdx, 108, (ByteBufProcessor) ByteProcessor.FIND_NUL);
            if (addressEnd == -1) {
                addressLen = 108;
            } else {
            ...
            addressEnd = header.forEachByte(startIdx, 108, (ByteBufProcessor) ByteProcessor.FIND_NUL);
            if (addressEnd == -1) {
                addressLen = 108;
            } else {
                addressLen = addressEnd - startIdx;
